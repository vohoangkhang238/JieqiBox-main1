...
<script setup lang="ts">
  ...
  async function checkAndTriggerAi() {
    ...
    // Nếu đang nghĩ nhưng không phải lượt AI, thì dừng
    if (isThinking.value && !isCurrentAiTurnNow() && !isManualAnalysis.value) {
      stopAnalysis({ playBestMoveOnStop: false })
      return
    }
    
    // Điều kiện khởi chạy AI:
    ...
    if (shouldRunAi) {
      console.log("checkAndTriggerAi: Starting AI move analysis (Infinite mode).")
      ...
      // Chế độ Infinite Analysis
      startAnalysis(infiniteSettings, engineMovesSinceLastReveal.value, baseFenForEngine.value, currentSearchMoves.value)
    }
  }

  // Watchers
  // Kích hoạt lại AI khi nước đi vừa xong hoặc trạng thái AI thay đổi
  watch([sideToMove, engineLoaded, pendingFlip], () => { nextTick(() => checkAndTriggerAi()) }, { immediate: true })

  // Watcher riêng cho AI toggles để đảm bảo trigger ngay cả khi không có thay đổi FEN/side
  watch([isRedAi, isBlackAi], () => {
    nextTick(() => {
  ...
  watch(currentMoveIndex, () => {
    // Nếu đang ở chế độ phân tích thủ công (không phải AI chơi), tự động khởi động lại phân tích sau mỗi nước đi
  ...
  watch(bestMove, move => {
    ...
    if (engineLoaded.value && isCurrentAiTurnNow() && !isMatchRunning.value && !isManualAnalysis.value) {
      ...
      // FIX 1: Gửi lệnh stop trước khi đi để Engine thoát khỏi trạng thái thinking (buộc Engine dừng tìm kiếm)
      stopAnalysis({ playBestMoveOnStop: false }); 
      ...
      setTimeout(() => {
        ...
        if (ok) {
          ...
          // Luôn gọi checkAndTriggerAi để đảm bảo AI đối thủ (hoặc chính nó) đi tiếp
          nextTick(() => checkAndTriggerAi()) 
        }
      }, 50)
    }
  ...
  onMounted(async () => {
    ...
    // FIX: Thêm listener để ngăn tắt phân tích thủ công khi đi nước
    const originalForceStopHandler = (event: CustomEvent) => {
      // Chỉ can thiệp nếu đang ở chế độ phân tích thủ công
      if (isManualAnalysis.value) {
        if (event.detail?.reason === 'manual-move') {
             console.log('[DEBUG] Manual move detected. Preventing force stop.')
             // Ngăn sự kiện gốc nếu có
    ...
    // Gắn listener
    window.addEventListener('force-stop-ai', originalForceStopHandler as EventListener, { capture: true });
    
    // Cleanup
    onUnmounted(() => {
        window.removeEventListener('force-stop-ai', originalForceStopHandler as EventListener, { capture: true });
    });
