...
export function useUciEngine(generateFen: () => string, gameState: any) {
  ...
  // --- MỚI: State lưu trữ options đã parse để UI sử dụng ---
  ...
  /* ---------- MỚI: Parsing Logic & setOption ---------- */
  // Hàm phân tích chuỗi options text thành mảng object cho UI
  ...
  // Hàm được gọi từ UI để set Threads/Hash
  ...
  const processPendingOutput = () => {
    ...
    pendingOutputLines.forEach(raw_ln => {
      ...
      // MỚI: Parse options khi nhận uciok
      if (ln === 'uciok') {
        parseUciOptionsToState() 
  ...
  const loadEngine = async (engine: ManagedEngine) => {
    ...
    uciOptions.value = [] // MỚI: Reset options
  ...
  const applySavedSettings = async () => {
    ...
    try {
      ...
      Object.entries(savedUciOptions).forEach(([name, value]) => {
        ...
        // MỚI: Sử dụng setOption để đồng bộ state
  ...
  onMounted(async () => {
    ...
    unlisten = await listen<string>('engine-output', ev => {
      ...
      // console.log(`[DEBUG] ENGINE_RAW_OUTPUT: ${raw_ln}`) // Comment bớt log cho đỡ spam
