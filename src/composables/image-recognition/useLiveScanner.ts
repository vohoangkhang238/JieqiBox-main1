...
export function useLiveScanner() {
  ...
  const startScanning = async (video: HTMLVideoElement, onDetected: (fen: string, boxes: any[]) => void) => {
    ...
    const loop = async () => {
      ...
      if (boxes.length > 0) {
        ...
        if (currentFen !== lastFen.value && currentFen.length > 20) {
          ...
          onDetected(currentFen, boxes)
        }
      }
      if (isScanning.value) setTimeout(loop, 300)
    }
    loop()
  }
