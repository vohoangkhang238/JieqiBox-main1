...
export function useLiveScanner() {
  ...
  const startScanning = async (video: HTMLVideoElement, onDetected: (fen: string) => void) => {
    ...
    console.log("Scanner: Bắt đầu quét...");

    const loop = async () => {
      if (!isScanning.value) return
      const boxes = await processLiveFrame(video)
      
      // Nếu thấy ít nhất 1 quân cờ hoặc bàn cờ thì mới xử lý
      if (boxes.length > 0) {
        const fen = gridToFen(updateBoardGrid(boxes))
        if (fen !== lastFen.value) {
          console.log("Scanner: Cập nhật FEN ->", fen)
          lastFen.value = fen
          onDetected(fen)
        }
      }
      ...
      if (isScanning.value) {
        setTimeout(loop, 200) // Quét nhanh 5 FPS
      }
    }
    loop()
  }
